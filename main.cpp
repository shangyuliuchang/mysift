...
int point = 0;
VideoCapture cap;
Mat img, pic, descriptor, descriptor2, raw, preprocess, *devicemat;
			...
			if (DRAW_DIRECTION) {
				for (int r = 0; r < 20; r++) {
					...
					if (x >= 0 && x < mHeight && y >= 0 && y < mWidth) {
						image[x * mWidth * 6 + y * 6 + 3] = (float)((matchNo * 10) % 256) / 255.0f;
						image[x * mWidth * 6 + y * 6 + 4] = 1.0f - (float)((matchNo * 10) % 256) / 255.0f;
						image[x * mWidth * 6 + y * 6 + 5] = (float)((matchNo * 10) % 256) / 255.0f;
					}
				}
			}
			if (DRAW_POINT) {
